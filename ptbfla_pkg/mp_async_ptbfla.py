...
class PtbFla:
    ...
    async def start(self) -> None:
        ...
        server_task = asyncio.create_task(asyncio.start_server(server_fun, self.myIpAdr, self.myPort))
        await server_task
        ...
        if self.nodeId == 0:
            ...
            await asyncio.sleep(1) # This cannot be called form a non-async function like the constructor
            await sendMsg((self.masterIpAdr, 6000), [self.nodeId, self.myIpAdr, self.myPort])
        ...
        self.remoteServerAddresses = [adr for adr in self.allServerAddresses if adr != self.localServerAddress]
    ...
    async def fl_centralized(self, fl_cent_server_processing, fl_cent_client_processing, localData, privateData, noIterations=1):
    ...
    async def fl_decentralized(self, fl_decent_server_processing, fl_decent_client_processing, localData, privateData, noIterations=1):
        # Save initial localData and privateData
        self.localData = localData
        self.privateData = privateData
        ...
        for iterNo in range(noIterations):
            ...
            await broadcastMsg(self.remoteServerAddresses, [iterNo, PHASE1, self.localServerAddress, self.localData])
            ...
            while len(dataFromClients1) > 0:
                ...
                await sendMsg(tuple(msg[msgSrcAdr]), [iterNo, PHASE2, self.localServerAddress, tmpLocalData])
    ...
    async def get1Meas(self, peerId, odata):
        ...
        await sendMsg(self.allServerAddresses[peerId], [self.timeSlot, self.nodeId, odata])
