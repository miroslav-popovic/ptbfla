#
#   Example 5: Decentralized HFL using the logistic regression model
...
def fl_cent_client_processing(localData, privateData, msg):
    ...
    b1 = msg[1]
    epochs = 300
    b0, b1 = logistic_regression(X_train, y_train, b0, b1, 0.001, epochs)
    return [b0, b1]
...
# privateData = [X_train, y_train]
# msgs = [[b00, b01], [b10, b11], ...]. In bij: i - partion (or client) index, j - coefficent index.
def fl_decent_server_processing(privateData, msgs):
    myData = fl_cent_client_processing(None, privateData, [0., 0.])
    msgs2 = msgs + [myData]
    myData2 = fl_cent_server_processing(None, msgs2)
    return myData2

from ptbfla_pkg.ptbfla import *

#   Run this example (after: cd src/examples): launch example5_dec_log_regression.py 2 id
def main():
    # Parse command line arguments
    if len(sys.argv) != 3:
        # Args: noNodes inodeId
        #   noNodes - number of nodes, nodeId - id of a node
        print('Program usage: python example5_dec_log_regression.py noNodes nodeId')
        print('Example: noNodes==2, nodeId=0..1, i.e. 2 nodes (id=0,1):')
        print('python example5_dec_log_regression.py 2 0',
              '\npython example5_dec_log_regression.py 2 1')
        exit()
    ...
    # Call fl_decentralized with noIterations = 1
    ret = ptb.fl_decentralized(fl_decent_server_processing, fl_cent_client_processing, lData, pData, 1)
