...
#   TODO
#
#   Suggestion - to spped-up the simulation:
#       Before running examples with many nodes (e.g. 170):
#           1. disnonnect from Internet, and
#           2. exit all apps.
#
#   Run this example:
#       1. First change directory with this command: cd src/examples
#       2. For the case 2 Svs (2 SVs, 1 blok, 1 t_slots) enter: launch example6_odts.py 2 id 1 1
#       3. For the case 10 Svs (10 SVs, 1 blok, 3 t_slots) enter: launch example6_odts.py 10 id 1 3
#       4. For the case 170 Svs (170 SVs, 1 blok, 25 t_slots) enter: launch example6_odts.py 170 id 1 25
#
...
def main():
    ...
    if len(sys.argv) != 5:
        # Args:
        #   no_nodes - number of nodes, node_id - id of a node, N - number of blocks, n - number of time slots
        print('Program usage: python example6_odts.py no_nodes node_id N n')
    ...
    # Set the number of blocks N (i.e., the number of iterations) and the number of time slots, or just slots n
    N = int( sys.argv[3] )
    n = int( sys.argv[4] )
    ...
    for block_number in range(N): # iterate over blocks; block_number counts from 0 to N-1
        ...
        for t_slot in range(n):   # iterate over time slots
            ...
            obs = ptb.get1Meas(peer_id, odata) # simulate measurements
